...
class Generator extends \yii\gii\Generator
{
    ...
    public function getYiiAlias($replace_symbol = true){
        $delimiter = '/';
        $arr = explode($delimiter, str_replace('\\', '/', $this->modelClass));
        $check = Yii::$aliases;
        $alias = '';
        $key = '@' . $arr[0];
        while (is_array($check) && array_key_exists($key, $check)) {
            $check = $check[$key];
            unset($arr[0]);
        ...
        if($replace_symbol == true) $alias = str_replace('/', '\\', $alias);
        return $alias;
    }
    ...
    public function afterValidate()
    {
        ...
        if($this->enablePjax == false) $this->messageCategory = 'app';
    }
}
